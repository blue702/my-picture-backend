package com.my.mypicturebackend.manager.auth;
...
import com.my.mypicturebackend.model.enums.SpaceRoleEnum;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        // 将其转换为 ServletRequestAttributes 类型以获取实际的 HttpServletRequest 对象
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
        String contentType = request.getHeader(Header.CONTENT_TYPE.getValue());
        SpaceUserAuthContext authRequest;

        // 兼容 get 和 post 操作
        // 首先检查请求的内容类型（Content-Type）
        // 如果是JSON格式请求（如POST请求），则从请求体中读取JSON数据并转换为 SpaceUserAuthContext 对象
        // 如果是非JSON格式请求（如GET请求），则从请求参数中获取参数映射并转换为 SpaceUserAuthContext 对象
        ...
        if (ObjUtil.isNotNull(id)) {
            ...
            // 去除上下文路径，获取剩余部分，比如 "/api/picture/aaa?id=1" -> "picture/aaa?id=1"
            String partUri = requestUri.replace(contextPath + "/", "");
            // 获取模块名称，比如 "picture/aaa?id=1" -> "picture"
